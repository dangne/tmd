...
class ASCCXLNetModel(XLNetPreTrainedModel, nn.Module):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
    ...
    def forward(self, input_ids, attention_mask, token_type_ids):
        ...
        # here we need to calculate clean logits with no grad, to find adv examples
        with torch.no_grad():
            clean_transformer_outputs = self.transformer(input_ids, attention_mask, token_type_ids=token_type_ids)
            clean_output = clean_transformer_outputs[0]
            clean_output = self.sequence_summary(clean_output)
            clean_logits = self.logits_proj(clean_output)
        ...
        for _ in range(self.num_steps):
            ...
            with torch.enable_grad():
                ...
                adv_transformer_outputs = self.transformer(attention_mask=attention_mask, token_type_ids=token_type_ids, inputs_embeds=embed_adv)
                adv_output = adv_transformer_outputs[0]
                adv_output = self.sequence_summary(adv_output)
                adv_logits = self.logits_proj(adv_output)
...
class ASCCAlbertModel(AlbertPreTrainedModel, nn.Module):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
    ...
    def forward(self, input_ids, attention_mask, token_type_ids):
        ...
        # here we need to calculate clean logits with no grad, to find adv examples
        with torch.no_grad():
            clean_outputs = self.albert(input_ids, attention_mask, token_type_ids)
            pooled_clean_output = self.dropout(clean_outputs[1])
            clean_logits = self.classifier(pooled_clean_output)
        ...
        for _ in range(self.num_steps):
            ...
            with torch.enable_grad():
                w_after_mask = weighting_mask.unsqueeze(-1) * w + ~weighting_mask.unsqueeze(-1) * -999
                embed_adv = torch.sum(input_nbr_embed * F.softmax(w_after_mask, -2) * weighting_mask.unsqueeze(-1), dim=2)
                adv_outputs = self.albert(attention_mask=attention_mask, token_type_ids=token_type_ids,
                                        inputs_embeds=embed_adv)
                pooled_adv_output = self.dropout(adv_outputs[1])
                adv_logits = self.classifier(pooled_adv_output)
                adv_loss = - self.criterion_kl(F.log_softmax(adv_logits, dim=1),
